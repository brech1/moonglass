...
pub(crate) type Result<T> = std::result::Result<T, FixtureError>;
...
fn valid_fixture_stem(stem: &str) -> std::result::Result<(), &'static str> {
    if stem.is_empty() {
...
pub(crate) fn read_meta(case_dir: &Path) -> Result<Meta> {
    ...
    let text = match std::fs::read_to_string(&path) {
        Ok(text) => text,
        Err(e) if e.kind() == std::io::ErrorKind::NotFound => return Ok(Meta::default()),
        Err(e) => {
...
mod tests {
    use super::*;
    use crate::testing::{
        BLS_DISABLED_ATTESTATION, SANITY_BLOCK_INVALID_OLD_STYLE_DEPOSIT_REJECTED, SLOTS_1,
    };

    #[test]
    fn meta_parses_blocks_count_from_checked_in_vector() {
        let m =
            read_meta(&SANITY_BLOCK_INVALID_OLD_STYLE_DEPOSIT_REJECTED.root()).expect("read meta");
        assert_eq!(m.blocks_count, Some(1));
        assert_eq!(m.bls_setting, None);
    }

    #[test]
    fn meta_parses_bls_setting_from_checked_in_vector() {
        let m = read_meta(&BLS_DISABLED_ATTESTATION.root()).expect("read meta");
        assert_eq!(m.bls_setting, Some(BlsSetting::Disabled));
    }

    #[test]
    fn read_meta_returns_default_when_checked_in_case_has_no_meta() {
        let m = read_meta(&SLOTS_1.root()).expect("read");
        assert!(m.blocks_count.is_none());
        assert!(m.bls_setting.is_none());
    }

    #[test]
    fn manifest_matches_checked_in_case_identity() {
        let case = SLOTS_1.to_case();

        validate_case_manifest(&case).expect("manifest should match asset path");
    }

    #[test]
    fn manifest_mismatch_reports_real_manifest_field() {
        let mut case = SLOTS_1.to_case();
        case.id = "not_slots_1".to_owned();

        let err = validate_case_manifest(&case).expect_err("manifest mismatch should fail");
        assert!(matches!(
            err,
            FixtureError::ManifestMismatch {
                ...
                got,
                want
            } if path.ends_with("manifest.yaml") && got == "slots_1" && want == "not_slots_1"
        ));
    }

    #[test]
    fn fixture_stem_rejects_path_syntax() {
        let err = serde_yaml::from_str::<FixtureStem>("'../block_0'").expect_err("reject path");
        assert!(
            err.to_string()
                .contains("fixture stem may contain only ASCII")
        );
    }
}
