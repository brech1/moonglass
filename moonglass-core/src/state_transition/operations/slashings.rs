...
use crate::state_transition::{BeaconStateLookup, compute_signing_root, verify_signature};
...
impl BeaconState {
    ...
    pub fn process_proposer_slashing(
        ...
        if !proposer.is_slashable_validator(current_epoch) {
    ...
    pub fn process_attester_slashing(
        ...
        let set1: BTreeSet<u64> = a1.attesting_indices.iter().map(|i| i.as_u64()).collect();
        let set2: BTreeSet<u64> = a2.attesting_indices.iter().map(|i| i.as_u64()).collect();
        ...
        for raw in intersection {
            ...
            if v.is_slashable_validator(epoch) {
