...
impl BeaconState {
    ...
    /// State-aware signing domain for `domain_type` at `epoch`, or now if omitted.
    pub fn get_domain(
        &self,
        domain_type: DomainType,
        epoch: Option<Epoch>,
    ) -> Result<Domain, TransitionError> {
        let epoch = epoch.unwrap_or(self.slot.epoch());
        compute_domain(
            domain_type,
    ...
    /// State-aware signing domain for callers that pass an explicit epoch.
    pub fn domain_for(
        &self,
        domain_type: DomainType,
        epoch: Epoch,
    ) -> Result<Domain, TransitionError> {
        self.get_domain(domain_type, Some(epoch))
    }
    ...
    where
        T: Merkleized,
    {
        let domain = self.get_domain(domain_type, Some(epoch))?;
