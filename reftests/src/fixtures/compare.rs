...
mod tests {
    ...
    #[test]
    fn diff_lists_changed_top_level_fields() {
        let mut got = BeaconState::default();
        let mut want = BeaconState {
            slot: Slot::new(1),
            ..BeaconState::default()
        };

        let detail = diff(&mut got, &mut want)
            .expect("hash roots")
            .expect("states differ");
        assert!(detail.contains("state root mismatch"));
        assert!(detail.contains("differing fields: slot"));
    }
}
