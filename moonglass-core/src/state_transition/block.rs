...
impl BeaconState {
    ...
    ///
    /// The first phase, [`BeaconState::process_parent_execution_payload`],
    ...
    pub fn process_block(&mut self, block: &BeaconBlock) -> Result<(), TransitionError> {
        ...
        self.process_parent_execution_payload(block)?;
    ...
    /// proposer, and the `parent_root` must hash-match the cached parent
    /// header. Once those identity checks hold, the block's header is stored as
    /// `latest_block_header` with a zero state root, which a later
    /// [`BeaconState::process_slot`] backfills. The proposer is then checked for
    /// slashing, matching the inherited helper's direct mutation order while the
    /// clone-commit block entry point still discards the scratch state on
    /// failure.
    ...
    pub fn process_randao(&mut self, body: &BeaconBlockBody) -> Result<(), TransitionError> {
        ...
        let epoch_object = epoch;
        let signing_root =
            self.signing_root_for(&epoch_object, DOMAIN_RANDAO, epoch, MerkleError::Epoch)?;
        verify_signature(
            &pubkey,
            signing_root,
            &body.randao_reveal,
    ...
    pub fn process_eth1_data(&mut self, body: &BeaconBlockBody) -> Result<(), TransitionError> {
        ...
        self.eth1_data_votes
            .push(body.eth1_data)
            .map_err(|_| BlockError::Eth1VotesFull)?;
        let votes_for = self
            .eth1_data_votes
            .iter()
