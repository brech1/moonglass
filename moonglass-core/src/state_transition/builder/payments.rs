//! [Builder payment window](crate::glossary#builder-payment-window) accounting
...
impl BeaconState {
    ...
    pub fn get_builder_payment_quorum_threshold(&self) -> Result<Gwei, TransitionError> {
        let per_slot_balance = self.get_total_active_balance()?.as_u64() / SLOTS_PER_EPOCH as u64;
        let quorum = per_slot_balance
            .checked_mul(BUILDER_PAYMENT_THRESHOLD_NUMERATOR)
            .ok_or(TransitionError::ArithmeticOverflow(
                TransitionArithmetic::Weight,
            ))?;
        Ok(Gwei(quorum / BUILDER_PAYMENT_THRESHOLD_DENOMINATOR))
    }
    ...
    /// [`BeaconState::can_builder_cover_bid`] holds back when checking a new
    /// bid.
    ...
    /// [`BeaconState::get_builder_payment_quorum_threshold`] is queued onto
    ...
    pub fn settle_builder_payment_if_quorum(
        ...
        if payment.weight >= self.get_builder_payment_quorum_threshold()? {
            self.queue_builder_pending_withdrawal(payment.withdrawal)?;
        }
        Ok(())
    }
    ...
    /// hard cap.
    pub fn queue_builder_pending_withdrawal(
        &mut self,
        ...
        self.builder_pending_withdrawals
            .push(withdrawal)
            .map_err(|_| OperationError::BuilderPendingWithdrawalsFull)?;
        Ok(())
    }
}
