...
pub(crate) type Result<T> = StdResult<T, WorkerError>;
...
pub(crate) fn internal_case_worker(args: &[String]) -> bool {
    ...
    flag == WORKER_ARG
        && env::var(WORKER_ENV).is_ok_and(|value| value == WORKER_ENV_VALUE)
        && env::var(WORKER_TOKEN_ENV).is_ok_and(|value| value == *token)
}

pub(crate) fn run_case_worker() -> Result<()> {
    let request: protocol::Request = serde_json::from_reader(stdin())
...
fn run_case_process(case: &Case, trace: TraceMode) -> Result<WorkerReport> {
    ...
    let mut child =
        Command::new(env::current_exe().map_err(|source| WorkerError::CurrentExe { source })?)
            .arg(WORKER_ARG)
            .arg(&token)
...
fn worker_token() -> String {
    ...
    format!("{}-{next}", process::id())
}

fn abort_worker(mut child: Child) {
    let _ = child.kill();
    let _ = child.wait();
}
...
fn decode_worker_output(output: Output) -> Result<WorkerReport> {
    ...
    let json = str::from_utf8(&json).map_err(|source| WorkerError::WorkerOutcomeUtf8 {
        source,
...
fn split_worker_json_payload(mut stdout: Vec<u8>) -> StdResult<(Vec<u8>, Vec<u8>), Vec<u8>> {
