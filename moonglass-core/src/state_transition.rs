...
//! consensus-side commitments. Fork choice handles data availability and the
//! execution verifier before recording a delivered payload.
//!
//! # Reading routes
//!
//! Block transition: [`BeaconState::state_transition`] clones the pre-state,
//! runs [`BeaconState::process_slots`], verifies the proposer signature, applies
//! [`BeaconState::process_block`], checks the claimed post-state root, then
//! commits the clone back to `self`.
//!
//! Slot transition: [`BeaconState::process_slots`] repeatedly calls
//! [`BeaconState::process_slot`], which records recent roots, advances the clock,
//! clears the payload-availability bit for the next slot, and runs
//! [`BeaconState::process_epoch`] at epoch boundaries.
//!
//! Block body transition: [`BeaconState::process_block`] handles the previous
//! slot's parent payload commitment before accepting the current slot's builder
//! bid and operations.
//!
//! Operation transition: [`BeaconState::process_operations`] runs slashings,
//! beacon attestations, exits, credential changes, and payload attestations in
//! consensus order.
//!
...
use crate::constants::{
    BUILDER_INDEX_SELF_BUILD, DOMAIN_BEACON_BUILDER, DOMAIN_BEACON_PROPOSER, GENESIS_SLOT,
    SLOT_DURATION_MS,
};
...
use crate::primitives::{BLSPubkey, BuilderIndex, Root, Slot};
use crate::ssz::Merkleized;

#[doc(hidden)]
pub trait TreeRootExt {
    /// Compute the SSZ tree root, emitting `on_fail` on merkleization failure.
    fn tree_root(&self, on_fail: MerkleError) -> Result<Root, TransitionError>;
}
...
{
    fn tree_root(&self, on_fail: MerkleError) -> Result<Root, TransitionError> {
        Merkleized::hash_tree_root(self)
            .map(Root::from)
...
impl BeaconState {
    ...
    /// Compatibility wrapper for callers that name the transition by input.
    pub fn apply_signed_block(
        &mut self,
        signed_block: &SignedBeaconBlock,
    ) -> Result<(), TransitionError> {
        self.state_transition(signed_block)
    }
    ...
    pub fn current_block_root(&self) -> Result<Root, TransitionError> {
        let state_root = self.tree_root(MerkleError::BeaconState)?;
        let header = self.latest_block_header.with_state_root(state_root);
    ...
    pub fn verify_execution_payload_envelope_signature(
        ...
        let envelope_msg = envelope.clone();
    ...
    pub fn execution_payload_envelope_signer(
        &self,
    ...
    pub fn validate_execution_payload_envelope(
        ...
        let expected_timestamp = self.compute_time_at_slot(self.slot)?;
    ...
    /// Return the timestamp at `slot`.
    pub fn compute_time_at_slot(&self, slot: Slot) -> Result<u64, TransitionError> {
        // Multiply slot * SLOT_DURATION_MS first and divide by 1000 last so the
        // result is exact when SLOT_DURATION_MS is not a multiple of 1000.
        let slots_since_genesis = slot.as_u64().saturating_sub(GENESIS_SLOT.as_u64());
        slots_since_genesis
    ...
    pub fn verify_block_signature(
        ...
        let signing_root = compute_signing_root(&block, domain, MerkleError::BeaconBlock)?;
        verify_signature(
            &pubkey,
            signing_root,
    ...
    pub fn expect_post_state_root(&mut self, expected: Root) -> Result<(), TransitionError> {
