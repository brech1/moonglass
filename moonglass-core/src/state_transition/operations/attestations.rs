...
use crate::error::{
    BlockError, BoundedList, MerkleError, OperationError, SignatureError, StateTransitionInvariant,
    TransitionArithmetic, TransitionError,
};
use crate::primitives::{BLSPubkey, Gwei, Root, Slot, ValidatorIndex};
use crate::ssz::List;
use crate::state_transition::balance::integer_squareroot;
...
impl BeaconState {
    ...
    pub fn attesting_indices(
        ...
        if attestation.aggregation_bits.len() != offset {
    ...
    pub fn indexed_attestation(
        ...
        let mut indices = List::<ValidatorIndex, MAX_ATTESTING_INDICES>::default();
        for vi in attesting {
            indices.push(vi).map_err(|_| {
                TransitionError::BoundedListFull(BoundedList::IndexedAttestationIndices)
            })?;
        }
    ...
    pub fn validate_indexed_attestation(
        ...
        let signing_root = compute_signing_root(&data, domain, MerkleError::AttestationData)?;
    ...
    pub fn get_attestation_participation_flag_indices(
        ...
        let is_matching_head = is_matching_target
            ...
            && payload_matches;
        let sqrt_slots = integer_squareroot(SLOTS_PER_EPOCH as u64);
        let mut out = Vec::new();
    ...
    pub fn accept_attestation(
        ...
        let earliest_inclusion_slot = data
            .slot
            .as_u64()
            .checked_add(MIN_ATTESTATION_INCLUSION_DELAY)
            .ok_or(TransitionError::ArithmeticOverflow(
                TransitionArithmetic::Slot,
            ))?;
        if earliest_inclusion_slot > self.slot.as_u64() {
        ...
        let participation_flags =
            self.get_attestation_participation_flag_indices(data, inclusion_delay)?;
    ...
    pub fn record_attestation_participation(
        ...
        for vi in &accepted.attesting_indices {
            ...
            for (flag_index, weight) in PARTICIPATION_FLAG_WEIGHTS.iter().enumerate() {
                ...
                let participation_index = vi.as_usize();
                let missing_participation = if accepted.target_is_current_epoch {
                    StateTransitionInvariant::MissingCurrentEpochParticipation(*vi)
                } else {
                    StateTransitionInvariant::MissingPreviousEpochParticipation(*vi)
                };
                let slot_flags = participation
                    .get_mut(participation_index)
                    .ok_or(missing_participation)?;
                if !slot_flags.has_flag(flag_index)? {
                    *slot_flags = slot_flags.with_flag(flag_index)?;
                    let reward_delta =
                    ...
                    proposer_reward_numerator =
                        proposer_reward_numerator.checked_add(reward_delta).ok_or(
    ...
    /// payment weight.
    pub fn record_builder_payment_weight_from_same_slot_attestation(
        &self,
    ...
    /// proposer.
    pub fn reward_attestation_proposer(
        &mut self,
...
pub fn indexed_attestation_from_known_indices(
    attestation: &Attestation,
    attesting_indices: &[ValidatorIndex],
) -> Result<IndexedAttestation, TransitionError> {
    let mut indexed_indices = List::<ValidatorIndex, MAX_ATTESTING_INDICES>::default();
    for vi in attesting_indices.iter().copied() {
        indexed_indices.push(vi).map_err(|_| {
            TransitionError::BoundedListFull(BoundedList::IndexedAttestationIndices)
        })?;
    }
    Ok(IndexedAttestation {
